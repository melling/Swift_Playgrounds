    ...
    */
    ...
    func processOpcodes(_ instructionPointer:Int) {
        ...
        if instructionPointer >= opcodes.count {
            print("ERROR: \(instructionPointer)")
            assertionFailure("ERROR: \(instructionPointer) > \(opcodes.count)")
        ...
        case 4: // output
            ...
            //let memoryPtr = opcodes[instructionPointer + 1]
            //let output = opcodes[memoryPtr]
            let output = getParameter(mode: parameter1Mode, instructionPointer: instructionPointer + 1)
            
            // Only the last one should be nonzero
            if output > 0 {
                print("Bad output: \(output) @ \(_pc) cmd=\(cmd), v=\(output)")
            }
            print("output=\(output)")
            processOpcodes(instructionPointer + 2)
            /*
